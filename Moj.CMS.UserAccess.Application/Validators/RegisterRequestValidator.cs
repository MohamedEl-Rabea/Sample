...
namespace Moj.CMS.UserAccess.Application.Validators
{
    ...
    {
        public RegisterRequestValidator(IStringLocalizer<UsersLocalizer> localizer)
        {
            RuleFor(request => request.UserName)
               .Must(x => !string.IsNullOrWhiteSpace(x)).WithMessage(x => localizer["UserName is required"])
               .Matches(@"^[A-Za-z\d-._@]+$").WithMessage(x => $"{localizer["UserName"]} {x.UserName} {localizer["is invalid, can only contain letters or digits"]}")
