...
namespace Moj.CMS.Domain.Shared.Guard
{
    public static class Guard
    {
        ...
        public static void AssertArgumentNotIncludeWhitespaces(string value, string argumentName)
        {
            AssertArgumentNotNullOrEmptyOrWhitespace(value, argumentName);
            if (value.Contains(" "))
                throw new ArgumentException("Value cannot be have white spaces.", argumentName);
        }

        public static void AssertArgumentNotNullOrEmptyOrWhitespace(string value, string argumentName, Func<bool> condition = null)
        {
        ...
        public static void AssertNullableArgumentNotLessThanOrEqualToZero(decimal? value, string argumentName)
        {
            if (value.HasValue)
            {
                if (value <= 0)
        ...
        public static void AssertGuidNullOrEmpty(Guid? value, string argumentName)
        {
            if (value != null && value != Guid.Empty)
            {

                throw new ArgumentException("Guid value should be empty", argumentName);
            }
        }
        ...
        public static void AssertPrecedesDate(DateTimeOffset value, DateTimeOffset dateToPrecede, string parameterName, string validationMessage = null)
        {
            if (value > dateToPrecede)
            {
                if (!string.IsNullOrWhiteSpace(validationMessage))
    ...
    public class EqualityComparer<T> : IEqualityComparer<T>
    {
        ...
        public int GetHashCode(T obj)
        {
            ...
            if (obj is ValueObject valueObject)
                return JsonSerializer.Serialize(valueObject).GetHashCode();
